...
//Authorize transaction on your behalf
...
function allowance(address _owner, address _spender) public view returns (uint256 remaining){
  ...
  function mint(address payable _to, uint value) external payable {
    require(_to != address(0), "transfer to address ZERO not allowed");
    require(msg.value >= 5e18, "Insufficient payment. Payment must be at least 5 ethers.");
    _totalSupply += value;
    _balance[_to] += value; 
  ...
  function burn(uint value) external {
      ...
      uint256 burnAmount = (value * 90) / 100; 
      uint256 transferAmount = value - burnAmount; 
        _balance[msg.sender] -= value;
        _totalSupply -= burnAmount;
        _balance[address(0)] += burnAmount; 
        _balance[msg.sender] += transferAmount; 
